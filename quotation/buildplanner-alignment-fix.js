(() => {
  ...
  style.textContent = `
    ...
    /* Simple Quotation: DO NOT CHANGE. */
    ...
    /* Detailed Quotation: exact requested proportions. */
    ...
    #quotationContent .detailed-quotation-table td:nth-child(1){text-align:left!important}
  ...
  function generateBuildQuotation() {
    ...
    if (type === 'simple') {
      ...
      let no = 1;
      if (data.prelim.length) { const s = data.prelim.reduce((a,i)=>a+i.amount,0); body += sectionRow('A. Preliminaries',3); body += `<tr class="border-b"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2">${data.prelim.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(s)}</td></tr>`; }
      if (data.structures.length) { const s = data.structures.reduce((a,i)=>a+i.amount,0); body += sectionRow('B. Structural Works',3); body += `<tr class="border-b"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2">${data.structures.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(s)}</td></tr>`; }
      const archRooms = data.rooms.filter(r => r.area > 0 && (data.archByRoom[r.roomId]||[]).length);
      if (archRooms.length) { body += sectionRow('C. Architectural Works',3); archRooms.forEach(r => { const a = data.archByRoom[r.roomId] || []; body += roomRow(r,3); body += `<tr class="border-b align-top"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2 leading-6">${a.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(data.roomSubtotals[r.roomId]||0)}</td></tr>`; }); }
      if (data.electrical.length) { const s = data.electrical.reduce((a,i)=>a+i.amount,0); body += sectionRow('D. Electrical Works',3); body += `<tr class="border-b"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2">${data.electrical.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(s)}</td></tr>`; }
      ...
      const archRooms = data.rooms.filter(r => r.area > 0 && (data.archByRoom[r.roomId]||[]).length);
      if (archRooms.length) { body += sectionRow('C. ARCHITECTURAL WORKS',6,!firstSection); firstSection=false; archRooms.forEach(r => { body += roomRow(r,6); body += rows(data.archByRoom[r.roomId]); body += `<tr class="tc-q-subtotal"><td colspan="5" class="py-3 px-2 text-right">${esc(r.label)} Subtotal</td><td class="py-3 px-2 text-right">${money(data.roomSubtotals[r.roomId]||0)}</td></tr>`; }); }
