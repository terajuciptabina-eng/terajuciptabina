(() => {
  const install = () => {
    ...
    style.textContent = `
      ...
      #quotationContent .simple-quotation-table th:nth-child(2),#quotationContent .simple-quotation-table td:nth-child(2){text-align:left!important}
      ...
      #quotationContent .detailed-quotation-table col:nth-child(6){width:15%!important}
      #quotationContent .detailed-quotation-table th:nth-child(1),#quotationContent .detailed-quotation-table td:nth-child(1),#quotationContent .detailed-quotation-table th:nth-child(2),#quotationContent .detailed-quotation-table td:nth-child(2){text-align:left!important}
      #quotationContent .detailed-quotation-table th:nth-child(3),#quotationContent .detailed-quotation-table td:nth-child(3),#quotationContent .detailed-quotation-table th:nth-child(4),#quotationContent .detailed-quotation-table td:nth-child(4),#quotationContent .detailed-quotation-table th:nth-child(5),#quotationContent .detailed-quotation-table td:nth-child(5),#quotationContent .detailed-quotation-table th:nth-child(6),#quotationContent .detailed-quotation-table td:nth-child(6){text-align:right!important}
      #quotationContent .simple-quotation-table th,#quotationContent .simple-quotation-table td,#quotationContent .detailed-quotation-table th,#quotationContent .detailed-quotation-table td{vertical-align:top;overflow-wrap:break-word}
      #quotationContent .tc-q-section td{background:#f3f4f6!important;font-weight:700!important;text-transform:uppercase;letter-spacing:.03em;border-top:2px solid #9ca3af!important;border-bottom:1px solid #d1d5db!important;text-align:left!important;padding-top:10px!important;padding-bottom:10px!important}
      #quotationContent .tc-q-room td{background:#fafafa!important;font-weight:700!important;color:#374151!important;text-align:left!important;border-top:1px solid #d1d5db!important;border-bottom:1px solid #e5e7eb!important;padding-top:9px!important;padding-bottom:9px!important}
    ...
    const sectionRow = (text,span,spacer) => `${spacer ? `<tr class="tc-q-spacer"><td colspan="${span}"></td></tr>` : ''}<tr class="tc-q-section"><td colspan="${span}" class="py-3 px-2">${text}</td></tr>`;
    const roomRow = (room,span) => `<tr class="tc-q-room"><td colspan="${span}" class="py-3 px-2">${esc(room?.label || 'Area / Room')} <span class="font-normal text-gray-500">(${qty(Number(room?.area)||0)} sqft)</span></td></tr>`;

    function generateBuildQuotation(){
      const rooms=typeof getRoomGroups==='function'?getRoomGroups():[];
      if(!rooms.some(r=>Number(r.area)>0)){alert('Please add at least one room with a valid area before generating the quotation.');return;}
      const data=typeof getCurrentQuotationData==='function'?getCurrentQuotationData():null;if(!data)return;
      ...
      const content=document.getElementById('quotationContent');if(!content)return;
      ...
      if(type==='simple'){
        ...
        let no=1;
        if(data.prelim.length){const s=data.prelim.reduce((a,i)=>a+i.amount,0);body+=sectionRow('A. Preliminaries',3)+`<tr class="border-b"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2">${data.prelim.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(s)}</td></tr>`}
        if(data.structures.length){const s=data.structures.reduce((a,i)=>a+i.amount,0);body+=sectionRow('B. Structural Works',3)+`<tr class="border-b"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2">${data.structures.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(s)}</td></tr>`}
        const archRooms=data.rooms.filter(r=>r.area>0&&(data.archByRoom[r.roomId]||[]).length);
        if(archRooms.length){body+=sectionRow('C. Architectural Works',3);archRooms.forEach(r=>{const a=data.archByRoom[r.roomId]||[];body+=roomRow(r,3)+`<tr class="border-b align-top"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2 leading-6">${a.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(data.roomSubtotals[r.roomId]||0)}</td></tr>`})}
        if(data.electrical.length){const s=data.electrical.reduce((a,i)=>a+i.amount,0);body+=sectionRow('D. Electrical Works',3)+`<tr class="border-b"><td class="py-3 px-2">${no++}</td><td class="py-3 px-2">${data.electrical.map(i=>esc(i.description)).join('<br>')}</td><td class="py-3 px-2 text-right font-semibold">${money(s)}</td></tr>`}
        ...
        let detailNo=1;const rows=arr=>(arr||[]).map(i=>`<tr class="border-b align-top"><td class="py-3 px-2">${detailNo++}</td><td class="py-3 px-2 text-left">${esc(i.description)}</td><td class="py-3 px-2 text-right">${esc(i.unit)}</td><td class="py-3 px-2 text-right">${qty(i.qty,i.unit)}</td><td class="py-3 px-2 text-right">${money(i.rate)}</td><td class="py-3 px-2 text-right font-medium">${money(i.amount)}</td></tr>`).join('');
        let first=true;
        if(data.prelim.length){body+=sectionRow('A. PRELIMINARIES',6,!first)+rows(data.prelim);first=false}
        if(data.structures.length){body+=sectionRow('B. STRUCTURAL WORKS',6,!first)+rows(data.structures);first=false}
        const archRooms=data.rooms.filter(r=>r.area>0&&(data.archByRoom[r.roomId]||[]).length);
        if(archRooms.length){body+=sectionRow('C. ARCHITECTURAL WORKS',6,!first);first=false;archRooms.forEach(r=>{body+=roomRow(r,6)+rows(data.archByRoom[r.roomId])+`<tr class="tc-q-subtotal"><td colspan="5" class="py-3 px-2 text-right">${esc(r.label)} Subtotal</td><td class="py-3 px-2 text-right">${money(data.roomSubtotals[r.roomId]||0)}</td></tr>`})}
        if(data.electrical.length){body+=sectionRow('D. ELECTRICAL WORKS',6,!first)+rows(data.electrical)}
      ...
      content.innerHTML=body;document.getElementById('quotationDocument')?.classList.remove('hidden');document.getElementById('quotationPrintActions')?.classList.remove('hidden');document.getElementById('quotationDocument')?.scrollIntoView({behavior:'smooth',block:'start'});
    }
