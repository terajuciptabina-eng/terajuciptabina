(() => {
  ...
  function install() {
    ...
    window.requestDetailedQuotation=function(){
      ...
      html+=`<div class="mb-5"><p class="text-sm text-gray-600">Construction works as described below. Final quotation is subject to site inspection, approved drawings, actual site conditions, specifications, material selection and confirmation of final scope of works.</p></div>`;
      ...
      html+=`<tr class="tc-q-summary-header"><td colspan="5" class="py-3 px-2">SUMMARY — TOTAL AMOUNT BY HIERARCHY</td></tr>`;
      summary.forEach(([title,total])=>{html+=`<tr class="tc-q-summary-row"><td colspan="4" class="py-2 px-2 text-left font-semibold">${title}</td><td class="py-2 px-2 text-right font-semibold whitespace-nowrap">${money2(total)}</td></tr>`;});
      html+=`</tbody><tfoot><tr class="border-t-2"><td colspan="4" class="py-4 px-2 text-left font-bold">TOTAL</td><td class="py-4 px-2 text-right font-bold text-lg whitespace-nowrap">${money2(data.total)}</td></tr></tfoot></table></div>`;
