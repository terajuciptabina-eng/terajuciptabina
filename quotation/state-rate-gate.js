/* TERAJU state-first quotation flow.
   ...
   For now state records use the same figures as Default Rate.
*/
(function(){
  'use strict';
  ...
  function loadResolver(){
    ...
    script.async=false;script.dataset.terajuStateRateResolver='1';
    document.head.appendChild(script);
  }

  function install(){
    const locationInput=document.getElementById('projectLocation');
    if(!locationInput || document.getElementById('projectState')){loadResolver();return;}
    ...
    field.parentElement.insertBefore(wrap.firstElementChild,field);
    ...
    function apply(){
      ...
      lock.classList.toggle('hidden',ok);lock.classList.toggle('flex',!ok);
      if(ok){setSelected(select.value);locationInput.removeAttribute('disabled');window.TERAJU_SELECTED_STATE=select.value;loadResolver();}
    }
    select.addEventListener('change',function(){
      if(!select.value){apply();return}
      setSelected(select.value);window.TERAJU_SELECTED_STATE=select.value;apply();
      try{window.dispatchEvent(new CustomEvent('teraju:statechange',{detail:{state:select.value}}))}catch(e){}
    });
    document.getElementById('stateFirstButton')?.addEventListener('click',()=>{select.focus();select.scrollIntoView({behavior:'smooth',block:'center'})});
    apply();
    loadResolver();
  }
