...
#quotationPreview .quotation-preview-loading{width:100%;padding:32px 20px;text-align:center;border:1px dashed #d1d5db;border-radius:16px;background:#fafafa;color:#6b7280}#quotationPreview .quotation-preview-page{position:relative!important}#quotationPreview .tc-column-resize-layer{position:absolute!important;pointer-events:none!important;z-index:50!important;box-sizing:border-box!important}#quotationPreview .tc-column-resizer{position:absolute;display:block;width:18px;cursor:col-resize;pointer-events:auto;touch-action:none;box-sizing:border-box;transform:translateX(-50%);background:rgba(255,255,255,.01)}#quotationPreview .tc-column-resizer::before{content:"";position:absolute;top:0;bottom:0;left:7px;width:3px;background:rgba(17,24,39,.32);border-radius:3px;box-shadow:0 0 0 1px rgba(255,255,255,.65)}#quotationPreview .tc-column-resizer::after{content:"↔";position:absolute;top:-18px;left:50%;transform:translateX(-50%);font:700 12px/16px Arial,sans-serif;color:#111827;background:#fff;border:1px solid #9ca3af;border-radius:4px;padding:0 3px;opacity:.9;box-shadow:0 1px 3px rgba(0,0,0,.15)}#quotationPreview .tc-column-resizer:hover::before,#quotationPreview .tc-column-resizer.is-dragging::before{background:#111827;width:4px}#quotationPreview .tc-column-resizer:hover::after,#quotationPreview .tc-column-resizer.is-dragging::after{opacity:1}body.tc-quotation-column-dragging{cursor:col-resize!important;user-select:none!important}body.tc-quotation-column-dragging *{user-select:none!important}
...
function quotationInstallResizeHandles(stage,pageRoots,meta,source){const sourceTable=quotationResizeSourceTable(source);if(!sourceTable)return;const saved=window.__tcQuotationColumnWidths&&window.__tcQuotationColumnWidths[quotationTableType(sourceTable)];if(saved)quotationApplyWidths(sourceTable,saved);stage.querySelectorAll('.tc-column-resize-layer').forEach(el=>el.remove());pageRoots.forEach((root,pi)=>{const m=meta[pi],page=stage.children[pi],img=page&&page.querySelector('img');if(!m||!page||!img)return;const install=function(){const ir=img.getBoundingClientRect(),pr=page.getBoundingClientRect(),scale=(ir.width||1)/RENDER_WIDTH_PX,layer=document.createElement('div');layer.className='tc-column-resize-layer';layer.style.left=(ir.left-pr.left)+'px';layer.style.top=(ir.top-pr.top)+'px';layer.style.width=ir.width+'px';layer.style.height=ir.height+'px';m.positions.forEach((pos,index)=>{const h=document.createElement('span');h.className='tc-column-resizer';h.style.left=(pos*scale)+'px';h.style.top=(m.top*scale)+'px';h.style.height=(m.height*scale)+'px';h.title='Drag to resize column';h.addEventListener('pointerdown',function(event){event.preventDefault();event.stopPropagation();const table=quotationResizeSourceTable(source),widths=quotationReadWidths(table),cols=table.querySelectorAll('colgroup col'),sx=event.clientX,l=cols[index].getBoundingClientRect().width,r=cols[index+1].getBoundingClientRect().width,minL=typeMinWidth(m.type,index),minR=typeMinWidth(m.type,index+1),maxD=r-minR,minD=minL-l;h.classList.add('is-dragging');document.body.classList.add('tc-quotation-column-dragging');const move=function(e){const d=Math.max(minD,Math.min(maxD,(e.clientX-sx)/scale));h.style.transform='translateX('+d+'px) translateX(-50%)'};const finish=async function(e){const d=Math.max(minD,Math.min(maxD,((e&&e.clientX)||sx)-sx)/scale),pair=widths[index]+widths[index+1],total=l+r,next=widths.slice();next[index]=(l+d)/total*pair;next[index+1]=(r-d)/total*pair;window.__tcQuotationColumnWidths=window.__tcQuotationColumnWidths||{};window.__tcQuotationColumnWidths[m.type]=next;quotationApplyWidths(table,next);document.body.classList.remove('tc-quotation-column-dragging');h.classList.remove('is-dragging');h.style.transform='translateX(-50%)';window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',finish);window.removeEventListener('pointercancel',finish);await renderQuotationPreview()};window.addEventListener('pointermove',move);window.addEventListener('pointerup',finish,{once:true});window.addEventListener('pointercancel',finish,{once:true})});layer.appendChild(h)});page.appendChild(layer)};if(img.complete&&img.naturalWidth)install();else img.addEventListener('load',install,{once:true})})}async function renderQuotationPreview(){injectStyles();const source=document.getElementById('quotationContent'),stage=document.getElementById('quotationPreview');if(!source||!stage)return;stage.innerHTML='<div class="quotation-preview-loading">Preparing visual quotation preview…</div>';try{await loadQuotationCanvas();const pageRoots=await buildPaginatedPages(source);const meta=quotationBuildResizeMeta(pageRoots);const pages=await rasterizePages(pageRoots);window.__quotationPreviewPages=pages.map(p=>p.src);window.__quotationPreviewPageMeta=pages;stage.innerHTML=pages.map((p,k)=>'<div class="quotation-preview-page"><img src="'+p.src+'" alt="Quotation page '+(k+1)+'"><div class="quotation-preview-page-number">Page '+(k+1)+' of '+pages.length+'</div></div>').join('');quotationInstallResizeHandles(stage,pageRoots,meta,source)}catch(error){console.error(error);stage.innerHTML='<div class="quotation-preview-loading">Unable to prepare the visual quotation preview. You can still try Download PDF.</div>'}}
...
function install(){injectStyles();bindQuotationTypeCards();window.renderQuotationPreview=renderQuotationPreview;window.printQuotation=printQuotation;window.TERAJU_QUOTATION_DOCUMENT_ENGINE_VERSION='2026-09-19-excel-column-resize-v9'}
