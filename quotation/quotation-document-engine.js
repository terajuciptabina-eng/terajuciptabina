...
.simple-quotation-table,.detailed-quotation-table{width:100%!important;table-layout:fixed!important;border-collapse:collapse!important}.simple-quotation-table th,.simple-quotation-table td,.detailed-quotation-table th,.detailed-quotation-table td{vertical-align:top!important}.simple-quotation-table th:first-child,.simple-quotation-table td:first-child,.detailed-quotation-table th:first-child,.detailed-quotation-table td:first-child{text-align:left!important}.simple-quotation-table th:last-child,.simple-quotation-table td:last-child,.detailed-quotation-table th:nth-child(4),.detailed-quotation-table td:nth-child(4),.detailed-quotation-table th:nth-child(5),.detailed-quotation-table td:nth-child(5),.detailed-quotation-table th:nth-child(6),.detailed-quotation-table td:nth-child(6){text-align:right!important}.simple-quotation-table th:nth-child(2),.simple-quotation-table td:nth-child(2),.detailed-quotation-table th:nth-child(2),.detailed-quotation-table td:nth-child(2),.detailed-quotation-table th:nth-child(3),.detailed-quotation-table td:nth-child(3){text-align:left!important}.simple-quotation-table td:nth-child(2),.detailed-quotation-table td:nth-child(2){overflow-wrap:anywhere;word-break:normal;white-space:normal}.simple-quotation-table td:last-child,.detailed-quotation-table td:nth-child(n+4){white-space:nowrap}
...
async function buildPaginatedPages(source){
 const working=source.cloneNode(true);working.classList.remove('quotation-render-source','quotation-locked');working.querySelectorAll('.no-print,#quotationPrintActions,#quotationLockOverlay').forEach(el=>el.remove());const children=Array.from(working.children);const tableWrap=children.find(el=>el.querySelector?.('table'));if(!tableWrap)return[working];const table=tableWrap.querySelector('table'),colgroup=table?.querySelector('colgroup'),thead=table?.querySelector('thead'),tbody=table?.querySelector('tbody'),tfoot=table?.querySelector('tfoot');if(!table||!tbody)return[working];const tableIndex=children.indexOf(tableWrap),headerNodes=children.slice(0,tableIndex),footerNodes=children.slice(tableIndex+1),template={wrapper:tableWrap,table,colgroup,thead,tfoot},units=collectPageUnits(tbody),roots=[];let page=makePageShell(headerNodes,template,true);const fits=p=>pageHeight(p)<=USABLE_HEIGHT_PX,hasBody=p=>!!p.tbody?.children.length,pushCurrent=()=>{if(hasBody(page))roots.push(page.root)},startNewPage=()=>{page=makePageShell(headerNodes,template,true)};
 for(const unit of units){const testRows=unit.map(r=>r.cloneNode(true));testRows.forEach(r=>page.tbody.appendChild(r));if(!fits(page)&&page.tbody.children.length>testRows.length){testRows.forEach(r=>r.remove());pushCurrent();startNewPage();appendUnits(page,[unit]);if(!fits(page))console.warn('Quotation row group exceeds one A4 page; keeping the row together may require smaller source text.')}}
 if(hasBody(page)&&tfoot){const total=tfoot.cloneNode(true);page.table.appendChild(total);if(!fits(page)){total.remove();pushCurrent();startNewPage();page.table.appendChild(tfoot.cloneNode(true))}}
 if(!hasBody(page)&&!roots.length)roots.push(page.root);
 let footerAttached=false;
 if(hasBody(page)&&footerNodes.length){
  const clones=footerNodes.map(node=>node.cloneNode(true));
  clones.forEach(node=>page.root.appendChild(node));
  if(fits(page)){footerAttached=true;roots.push(page.root)}
  else{clones.forEach(node=>node.remove());pushCurrent();const footerPage=makePageShell(headerNodes,template,false);footerNodes.forEach(node=>footerPage.root.appendChild(node.cloneNode(true)));roots.push(footerPage.root);footerAttached=true}
 }else if(hasBody(page))pushCurrent();
 if(!footerAttached&&footerNodes.length&&!roots.length){
  const footerPage=makePageShell(headerNodes,template,false);footerNodes.forEach(node=>footerPage.root.appendChild(node.cloneNode(true)));roots.push(footerPage.root);
 }
 return roots;
}
...
function install(){injectStyles();bindQuotationTypeCards();window.renderQuotationPreview=renderQuotationPreview;window.printQuotation=printQuotation;window.TERAJU_QUOTATION_DOCUMENT_ENGINE_VERSION='2026-09-09-alignment-v8'}
