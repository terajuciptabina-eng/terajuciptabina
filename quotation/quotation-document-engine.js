...
#quotationPreview .quotation-preview-page .quotation-preview-dom-root{width:794px!important;box-sizing:border-box!important;background:#fff!important;position:relative!important}
#quotationPreview .tc-column-resizer{position:absolute!important;top:0!important;right:-4px!important;width:8px!important;height:100%!important;z-index:30!important;cursor:col-resize!important;touch-action:none!important;background:transparent!important}
#quotationPreview .tc-column-resizer:hover,#quotationPreview .tc-column-resizer.is-dragging{background:rgba(17,24,39,.10)!important}
body.tc-quotation-column-dragging{cursor:col-resize!important;user-select:none!important}
body.tc-quotation-column-dragging *{user-select:none!important}
...
function createRenderRoot(){
 ...
 root.className='quotation-preview-dom-root';
...
function quotationDefaultWidths(table){return quotationTableType(table)==='detailed'?[6,42,9,11,16,16]:[8,72,20]}
function getQuotationTables(source,pageRoots){const tables=[];if(source){const t=source.querySelector('table.detailed-quotation-table,table.simple-quotation-table');if(t)tables.push(t)}(pageRoots||[]).forEach(root=>{const t=root.querySelector('table.detailed-quotation-table,table.simple-quotation-table');if(t)tables.push(t)});return tables}
function applyQuotationWidths(table,widths){const cols=table?.querySelectorAll('colgroup col');if(!cols||cols.length!==widths.length)return;cols.forEach((col,i)=>col.style.setProperty('width',String(widths[i])+'%','important'));table.style.tableLayout='fixed'}
function readQuotationWidths(table){const cols=Array.from(table?.querySelectorAll('colgroup col')||[]);if(!cols.length)return quotationDefaultWidths(table);const rects=cols.map(col=>col.getBoundingClientRect().width),total=rects.reduce((a,b)=>a+b,0);return total?rects.map(v=>v/total*100):quotationDefaultWidths(table)}
function installQuotationColumnResizers(pageRoots,source){const tables=getQuotationTables(source,pageRoots),byType={};tables.forEach(table=>{const type=quotationTableType(table);if(type)(byType[type]??=[]).push(table)});Object.entries(byType).forEach(([type,typeTables])=>{const saved=window.__tcQuotationColumnWidths?.[type],widths=saved&&saved.length===typeTables[0].querySelectorAll('colgroup col').length?saved:readQuotationWidths(typeTables[0]);typeTables.forEach(table=>applyQuotationWidths(table,widths));typeTables.forEach(table=>{Array.from(table.querySelectorAll('.tc-column-resizer')).forEach(el=>el.remove());Array.from(table.querySelectorAll('thead th')).forEach((th,index,ths)=>{if(index>=ths.length-1)return;th.style.position='relative';const handle=document.createElement('span');handle.className='tc-column-resizer no-print';handle.setAttribute('aria-label','Resize column');handle.addEventListener('pointerdown',event=>{event.preventDefault();event.stopPropagation();const current=readQuotationWidths(table),cols=table.querySelectorAll('colgroup col'),startX=event.clientX,startLeftPx=cols[index].getBoundingClientRect().width,startRightPx=cols[index+1].getBoundingClientRect().width,minLeftPx=type==='detailed'?[42,150,48,55,75][index]||42:[42,180][index]||42,minRightPx=type==='detailed'?[42,150,48,55,75,82][index+1]||42:[42,180,82][index+1]||42;handle.classList.add('is-dragging');document.body.classList.add('tc-quotation-column-dragging');const move=e=>{const delta=e.clientX-startX,maxDelta=startRightPx-minRightPx,minDelta=minLeftPx-startLeftPx,d=Math.max(minDelta,Math.min(maxDelta,delta)),totalWidth=table.getBoundingClientRect().width,next=current.slice();next[index]=(startLeftPx+d)/totalWidth*100;next[index+1]=(startRightPx-d)/totalWidth*100;window.__tcQuotationColumnWidths=window.__tcQuotationColumnWidths||{};window.__tcQuotationColumnWidths[type]=next;typeTables.forEach(t=>applyQuotationWidths(t,next));const srcTable=source?.querySelector(type==='detailed'?'table.detailed-quotation-table':'table.simple-quotation-table');if(srcTable)applyQuotationWidths(srcTable,next)};const up=()=>{handle.classList.remove('is-dragging');document.body.classList.remove('tc-quotation-column-dragging');window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);window.removeEventListener('pointercancel',up)};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true});window.addEventListener('pointercancel',up,{once:true})});th.appendChild(handle)})})})}
function renderQuotationPreviewPageShell(stage,pageRoot,index,total){const page=document.createElement('div');page.className='quotation-preview-page';page.appendChild(pageRoot);const number=document.createElement('div');number.className='quotation-preview-page-number no-print';number.textContent=`Page ${index+1} of ${total}`;page.appendChild(number);stage.appendChild(page)}
async function renderQuotationPreview(){injectStyles();const source=document.getElementById('quotationContent'),stage=document.getElementById('quotationPreview');if(!source||!stage)return;stage.innerHTML='<div class="quotation-preview-loading">Preparing visual quotation preview…</div>';try{await loadQuotationCanvas();const pageRoots=await buildPaginatedPages(source);window.__quotationPreviewRoots=pageRoots;installQuotationColumnResizers(pageRoots,source);stage.innerHTML='';pageRoots.forEach((root,i)=>renderQuotationPreviewPageShell(stage,root,i,pageRoots.length))}catch(error){console.error(error);stage.innerHTML='<div class="quotation-preview-loading">Unable to prepare the visual quotation preview. You can still try Download PDF.</div>}}
async function printQuotation(){const quotation=document.getElementById('quotationDocument');if(!quotation||quotation.classList.contains('hidden')){alert('Please generate the quotation first.');return}const button=document.querySelector('#quotationPrintActions button'),originalLabel=button?button.textContent:'';if(button){button.disabled=true;button.textContent='Preparing PDF…'}try{let roots=window.__quotationPreviewRoots||[];if(!roots.length){await renderQuotationPreview();roots=window.__quotationPreviewRoots||[]}if(!roots.length)throw new Error('Quotation preview is unavailable.');const pages=await rasterizePages(roots);window.__quotationPreviewPages=pages.map(p=>p.src);window.__quotationPreviewPageMeta=pages;await loadScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js',()=>!!(window.jspdf&&window.jspdf.jsPDF));const{jsPDF}=window.jspdf,pdf=new jsPDF({orientation:'p',unit:'mm',format:'a4',compress:true});pages.forEach((p,index)=>{if(index)pdf.addPage();const drawHeight=Math.min(CONTENT_MM.height,Math.max(.1,p.heightMm||CONTENT_MM.height));pdf.addImage(p.src,'JPEG',PAGE_MM.margin,PAGE_MM.margin,CONTENT_MM.width,drawHeight,undefined,'FAST');pdf.setFontSize(8);pdf.setTextColor(107,114,128);pdf.text(`Page ${index+1} of ${pages.length}`,PAGE_MM.width/2,PAGE_MM.height-4,{align:'center'})});const safeNumber=(typeof quotationNumber!=='undefined'&&quotationNumber?quotationNumber:'quotation').replace(/[^a-z0-9_-]+/gi,'-');pdf.save(`Quotation-${safeNumber}.pdf`)}catch(error){console.error(error);alert('Unable to create the PDF. Please check your internet connection and try again.')}finally{if(button){button.disabled=false;button.textContent=originalLabel||'Download PDF'}}}
...
function install(){injectStyles();bindQuotationTypeCards();window.renderQuotationPreview=renderQuotationPreview;window.printQuotation=printQuotation;window.TERAJU_QUOTATION_DOCUMENT_ENGINE_VERSION='2026-09-19-excel-column-resize-v1'}
