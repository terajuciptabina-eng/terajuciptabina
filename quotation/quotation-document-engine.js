...
function quotationBuildResizeMeta(pageRoots){return pageRoots.map(root=>{const table=root.querySelector('table.detailed-quotation-table,table.simple-quotation-table');if(!table)return null;const type=quotationTableType(table),rr=root.getBoundingClientRect(),tr=table.getBoundingClientRect();return{type,top:tr.top-rr.top,left:tr.left-rr.left,width:tr.width,height:tr.height,positions:Array.from(table.querySelectorAll('colgroup col')).slice(0,-1).map(col=>col.getBoundingClientRect().right-rr.left)}})}
