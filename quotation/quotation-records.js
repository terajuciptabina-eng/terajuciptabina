(() => {
  ...
  async async function loadQuotationForEdit(){ if(!currentQuotationId)return;try{const data=await fetchQuotations();const quotation=data.find(item=>item?.quotationId===currentQuotationId);if(!quotation){toast('Cost estimate record not found.',true);return}currentQuotationNumber=quotation.quotationNumber||'';currentQuotationType=quotation.quotationType||quotation.plannerState?.quotationType||'simple';currentProjectId=String(quotation.projectId||quotation.plannerState?.projectId||`LEGACY-${plannerType}-${quotation.quotationId}`);try{localStorage.setItem(projectStorageKey,currentProjectId)}catch{}const savedState=quotation.plannerState||{};const savedRooms=Array.isArray(savedState.rooms)?savedState.rooms:[];
      ...
      try{if(currentQuotationNumber&&typeof quotationNumber!=='undefined')quotationNumber=currentQuotationNumber}catch{}const roomsMatch=()=>{const cards=[...(document.querySelectorAll('#roomsContainer .room-card')||[])];if(cards.length!==savedRooms.length)return false;return savedRooms.every((roomData,index)=>String(cards[index]?.querySelector('.room-area')?.value??'')===String(roomData.area??'')&&String(cards[index]?.querySelector('.room-type')?.value??'')===String(roomData.type||'other'))};let verifyAttempts=0;const verifyRestore=()=>{if(!savedRooms.length||roomsMatch())return;if(verifyAttempts++>=20){console.warn('[TERAJU QUOTATION RESTORE] room restore did not stick',savedRooms);return}restoreSnapshot(savedState);setTimeout(verifyRestore,250)};setTimeout(verifyRestore,400);trackEvent('quotation_opened', {quotation_id:quotation.quotationId, quotation_number:currentQuotationNumber, quotation_type:currentQuotationType === 'detail' ? 'detailed' : 'simple'});toast(`Cost Estimate ${currentQuotationNumber||currentQuotationId} loaded.`)}catch(error){console.error(error);toast(error.message||'Unable to load cost estimate.',true)} }
