(() => {
  ...
  function snapshotPlanner() {
    const rooms = [...document.querySelectorAll('#roomsContainer .room-card')].map(room => ({id:room.id,type:room.querySelector('.room-type')?.value||'other',condition:room.querySelector('.room-condition')?.value||'existing',name:room.querySelector('.room-name')?.value||'',area:room.querySelector('.room-area')?.value||''}));
