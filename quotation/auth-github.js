(() => {
  ...
  function setMode(next) {
    mode = next; clearError(); generated.classList.add('hidden');
    ...
    idField.classList.toggle('hidden', signup); nameField.classList.toggle('hidden', !signup);
    emailInput?.parentElement.classList.toggle('hidden', !signup); phoneInput?.parentElement.classList.toggle('hidden', !signup);
    idInput.required = !signup; nameInput.required = signup; if (emailInput) emailInput.required = signup;
  ...
  function showPortal(record) {
    ...
    const id = record[idKey]; welcome.textContent = `Welcome, ${record.profile?.name || ''}. ${isHomeowner ? 'Homeowner ID' : 'Contractor ID'}: ${id}`;
  ...
  async function api(options) {
    const response = await fetch(`${API_BASE}/api/auth`, options).catch(() => null);
  ...
  form.addEventListener('submit', async event => {
    event.preventDefault(); event.stopImmediatePropagation(); clearError(); button.disabled = true;
    ...
    try {
      if (mode === 'signup') {
        ...
        const data = await api({ method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({role,name,email,phone}) });
        ...
        const id = idInput.value.trim().toUpperCase(); if (!id) throw new Error(`Please enter your ${isHomeowner ? 'Homeowner' : 'Contractor'} ID.`);
        const data = await api({ method:'GET', headers:{'Content-Type':'application/json'} });
        const remote = await fetch(`${API_BASE}/api/auth?role=${encodeURIComponent(role)}&id=${encodeURIComponent(id)}`);
        const result = await remote.json().catch(() => ({})); if (!remote.ok) throw new Error(result.message || 'Account not found.'); showPortal(result);
      }
