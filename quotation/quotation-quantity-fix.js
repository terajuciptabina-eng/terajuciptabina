...
function masterPathFromLegacyItem(i){const explicit=i?.masterPath||i?._masterPath;if(explicit)return explicit;const description=norm(i?.description);const unit=norm(i?.unit);if(!description)return'';for(const [p,r] of rulesByPath){const rd=norm(r?.[2]);const ru=norm(r?.[6]);if(rd&&rd===description&&(!unit||!ru||ru===unit))return p}return''}
function path(i){return masterPathFromLegacyItem(i)}
...
if(s.B>0){const ps=[['footing-conc','PORCH / Footing / Concrete','Footing','footing'],['footing-fw','PORCH / Footing / Formwork','Footing','footing'],['footing-rebar','PORCH / Footing / Rebar','Footing','footing'],['slab-conc','PORCH / Ground Slab / Concrete','Ground Slab','ground-slab'],['slab-brc','PORCH / Ground Slab / BRC','Ground Slab','ground-slab'],['gb-conc','PORCH / Ground Beam / Concrete','Ground Beam','ground-beam'],['gb-fw','PORCH / Ground Beam / Formwork','Ground Beam','ground-beam'],['gb-rebar','PORCH / Ground Beam / Rebar','Ground Beam','ground-beam'],['rb-conc','PORCH / Roof Beam / Concrete','Roof Beam','roof-beam'],['rb-fw','PORCH / Roof Beam / Formwork','Roof Beam','roof-beam'],['rb-rebar','PORCH / Roof Beam / Rebar','Roof Beam','roof-beam'],['col-conc','PORCH / Column / Concrete','Column','column'],['col-fw','PORCH / Column / Formwork','Column','column'],['col-rebar','PORCH / Column / Rebar','Column','column'],['fr-conc','PORCH / Flat Roof / Concrete','Flat Roof & Roofing','roof'],['fr-fw','PORCH / Flat Roof / Formwork','Flat Roof & Roofing','roof'],['fr-brc','PORCH / Flat Roof / BRC','Flat Roof & Roofing','roof'],['roof-m','PORCH / Roof / Metal Roofing Sheet','Roof','roof']];ps.forEach(([suf,p,t,k])=>add('porch-'+suf,p,t,k))}
return items}
