export default async function handler(req, res) {
  ...
  async function github(path,options={}){const r=await fetch(`https://api.github.com/repos/${actualRepo}/contents/${path}`,{...options,headers:{...headers,...(options.headers||{})}});const t=await r.text();let d=null;try{d=t?JSON.parse(t):null}catch{d=t}return{response:r,data:d};}
  async function read(id,type,qid){const x=await github(pathFor(id,type,qid));if(x.response.status===404)return{record:null,sha:null,status:404};if(!x.response.ok)return{record:null,sha:null,status:502};const content=x.data?.content?Buffer.from(x.data.content,'base64').toString('utf8'):'';return{record:content?JSON.parse(content):null,sha:x.data?.sha||null,status:200};}
  try{
    const source=req.method==='GET'||req.method==='DELETE'?req.query:(req.body||{});
    const id=clean(source?.id).toUpperCase(), type=clean(source?.plannerType).toLowerCase(), quotationId=clean(source?.quotationId);
    ...
    if(req.method==='DELETE'){
      ...
      const updated=await github(pathFor(id,type,quotationId),{method:'DELETE',body:JSON.stringify({message:`Delete project control ${quotationId}`,sha:current.sha})});
    ...
    const updated=await github(pathFor(id,type,quotationId),{method:'PUT',body:JSON.stringify(body)});
    if(!updated.response.ok){console.error('Project control write failed:',updated.data);return res.status(502).json({message:'Unable to save project control record.'});}
    return res.status(200).json({success:true,record});
  }catch(error){console.error('project control storage error:',error);return res.status(500).json({message:'Unable to process project control record.'});}
}