export default async function handler(req, res) {
  ...
  function nextBaseNumber(record, plannerType) { const all = plannerList(record, plannerType); const stored = Number(record?.quotationRunningNumber?.[plannerType]); const legacyStored = Number(record?.quotationRunningNumber); const highest = all.reduce((max, q) => Math.max(max, baseSequence(q?.quotationNumber)), 0); const baseline = Math.max(highest, Number.isFinite(stored) && stored > 0 ? stored : 0, Number.isFinite(legacyStored) && legacyStored > 0 ? legacyStored : 0, highest === 0 ? all.length : 0); return baseline + 1; }
  function nextEstimateNumber(record, plannerType) { const all = plannerList(record, plannerType); const stored = Number(record?.estimateRunningNumber?.[plannerType]); const highest = all.reduce((max, q) => Math.max(max, estimateSequence(q?.estimateNumber)), 0); return Math.max(highest, Number.isFinite(stored) && stored > 0 ? stored : 0) + 1; }
